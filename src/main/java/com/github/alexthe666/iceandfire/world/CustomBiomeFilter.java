...
public class CustomBiomeFilter extends PlacementFilter {
    ...
    protected boolean shouldPlace(final PlacementContext context, @NotNull final RandomSource random, @NotNull final BlockPos position) {
        ...
        if (!hasFeature) {
            ...
            BlockPos surfacePos = context.getLevel().getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, position);
            hasFeature = context.getLevel().getBiome(surfacePos).value().getGenerationSettings().hasFeature(placedfeature);
        }
