...
public class DummyPiece extends TemplateStructurePiece {
    ...
    public DummyPiece(StructureTemplateManager manager, Identifier location, BlockPos pos, Rotation rotation) {
        super(IafWorldRegistry.DUMMY_PIECE.get(), 0, manager, location, location.toString(), makeSettings(rotation), pos);
    }

    public DummyPiece(StructurePieceSerializationContext context, CompoundTag tag) {
        super(IafWorldRegistry.DUMMY_PIECE.get(), tag, context.structureTemplateManager(), identifier ->
            makeSettings(Rotation.valueOf(tag.getStringOr("Rot", Rotation.NONE.name()))));
    }

    private static StructurePlaceSettings makeSettings(Rotation rotation) {
        return new StructurePlaceSettings().setRotation(rotation).setMirror(Mirror.NONE)
            .setRotationPivot(new BlockPos(0, 0, 0)).addProcessor(BlockIgnoreProcessor.STRUCTURE_AND_AIR);
    }
