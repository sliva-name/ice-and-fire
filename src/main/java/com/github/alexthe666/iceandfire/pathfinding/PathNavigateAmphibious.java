...
public class PathNavigateAmphibious extends PathNavigation {
    ...
    protected boolean canUpdatePath() {
        return this.mob.onGround() || this.canFloat() && (this.mob.isInWater() || this.mob.isInLava()) || this.mob.isPassenger();
    }
    ...
    private int getPathablePosY() {
        if (this.mob.isInWater() && this.canFloat()) {
            ...
            Block block = this.level.getBlockState(BlockPos.containing(Mth.floor(this.mob.getX()), i, Mth.floor(this.mob.getZ()))).getBlock();
            int j = 0;

            while (block == Blocks.WATER) {
                ++i;
                block = this.level.getBlockState(BlockPos.containing(Mth.floor(this.mob.getX()), i, Mth.floor(this.mob.getZ()))).getBlock();
                ++j;
    ...
    private boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3 p_179692_7_, double p_179692_8_, double p_179692_10_) {
        for (BlockPos blockpos : BlockPos.betweenClosedStream(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1)).collect(Collectors.toList())) {
            ...
            if (d0 * p_179692_8_ + d1 * p_179692_10_ >= 0.0D) {
                ...
                if (IafMaterials.blocksMotion(this.level.getBlockState(blockpos))) {
                    return false;
                }
            }
        }
