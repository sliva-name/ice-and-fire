...
public class EntityMyrmexEgg extends LivingEntity implements IBlacklistedFromStatues, IDeadMob {
    ...
    public void addAdditionalSaveData(@NotNull ValueOutput tag) {
        ...
        tag.store("HiveUUID", UUIDUtil.CODEC, hiveUUID == null ? hiveUUID = UUID.randomUUID() : hiveUUID);
    }
        ...
        if (this.getMyrmexAge() > IafConfig.myrmexEggTicks) {
            ...
            if (myrmex instanceof EntityMyrmexQueen) {
                ...
                if (MyrmexWorldData.get(level()) != null) {
                    ...
                    if (!this.level().isClientSide() && hive != null && Math.sqrt(this.distanceToSqr(hive.getCenter().getX(), hive.getCenter().getY(), hive.getCenter().getZ())) < 2000) {
                        myrmex.setHive(hive);
                    }
                }
            }
    ...
    @Override
    public boolean hurtServer(@NotNull ServerLevel level, @NotNull DamageSource dmg, float var2) {
        if (dmg.is(DamageTypes.IN_WALL) || dmg.is(DamageTypes.FALL)) {
            return false;
        }
        if (!this.level().isClientSide() && !dmg.is(net.minecraft.tags.DamageTypeTags.BYPASSES_INVULNERABILITY)) {
            IafDrops.spawn(this, this.getItem(), 0);
        }
    ...
    private ItemStack getItem() {
        ...
        IafItemData.update(egg, tag -> tag.putInt("EggOrdinal", this.getMyrmexCaste()));
        return egg;
    }
