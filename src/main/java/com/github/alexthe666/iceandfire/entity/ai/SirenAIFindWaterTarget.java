            ...
            for (int x = posX - 5; x < posX + 5; x++) {
                for (int y = posY - 5; y < posY + 5; y++) {
                    for (int z = posZ - 5; z < posZ + 5; z++) {
                        if (mob.wantsToSing()) {
                            if (IafMaterials.isSolid(this.mob.level().getBlockState(new BlockPos(x, y, z))) && this.mob.level().isEmptyBlock(new BlockPos(x, y + 1, z)) && this.mob.isDirectPathBetweenPoints(this.mob.position(), new Vec3(x, y + 1, z))) {
                                singTargets.add(new Vec3(x, y + 1, z));
                            }
                        }
                        if (IafMaterials.isWater(this.mob.level().getBlockState(new BlockPos(x, y, z))) && this.mob.isDirectPathBetweenPoints(this.mob.position(), new Vec3(x, y, z))) {
