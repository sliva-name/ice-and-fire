...
public class EntityPixie extends TamableAnimal {
        ...
        if (this.isOwnedBy(player)) {
            ...
            if (!this.level().isClientSide()) {
                if (!this.getItemInHand(InteractionHand.MAIN_HAND).isEmpty()) {
                    IafDrops.spawn(this, this.getItemInHand(InteractionHand.MAIN_HAND), 0.0F);
                    this.stealCooldown = STEAL_COOLDOWN;
                }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            if (housePos != null && this.distanceToSqr(Vec3.atCenterOf(housePos)) < 1.5F && this.level().getBlockEntity(housePos) != null && this.level().getBlockEntity(housePos) instanceof TileEntityPixieHouse) {
                TileEntityPixieHouse house = (TileEntityPixieHouse) this.level().getBlockEntity(housePos);
                if (house.hasPixie) {
                    this.housePos = null;
                } else {
                    house.hasPixie = true;
                    ...
                    house.pixieOwnerUUID = IafOwners.getUUID(this);
