...
public class DragonType {
    ...
    public void updateEggCondition(EntityDragonEgg egg) {
        ...
        if (this == FIRE) {
            if (IafMaterials.isFire(egg.level().getBlockState(pos))) {
            ...
            if (egg.getDragonAge() > IafConfig.dragonEggTime) {
                ...
                IafOwners.setUUID(dragon, egg.getOwnerId());
        ...
        if (this == ICE) {
            if (IafMaterials.isWater(egg.level().getBlockState(pos)) && egg.getRandom().nextInt(500) == 0) {
        ...
        if (this == LIGHTNING) {
            ...
            if (egg.getDragonAge() > IafConfig.dragonEggTime) {
                ...
                dragon.setTame(true, false);
                IafOwners.setUUID(dragon, egg.getOwnerId());
                LightningBolt lightningboltentity = EntityTypes.LIGHTNING_BOLT.create(egg.level(), net.minecraft.world.entity.EntitySpawnReason.TRIGGERED);
